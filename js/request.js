...
async function make(){
    ...
    if(content){
        ...
        if(content[`request`][`french`]==false) {flag=``;}
...
async function makeWebGL(content,content_lang){
    ...
    const html=(`
        ...
        <div class='request-center iframe-container'><iframe src='${page+href}' frameborder='0' allow='autoplay; encrypted-media' allowfullscreen></iframe></div>
    `);
    $(`#home`).html(html);
}
