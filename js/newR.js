...
function select(){
    if(this.done==false){
        ...
        $.getJSON(this.project, function(data) {
            isProject(data);
        });
    }
...
function isProject(data){
    for(key in data.portfolio){
        if(key==this.q){
            //the project exist in common.json - portfolio
            this.done=true;
            makeProject(data.portfolio[key]);
            break;
        }
    }
}
...
function makeUnknown(){
    makeHeader("request-unknown-color",this.type,this.host);
    makeFooter("request-unknown-color",this.host);
    makeContent(this.alert,"request-unknown-color","titre","text","textbutton",this.to,this.host);
}
